    ...
    if dtype is None:
        try:
            ...
            dtype = (
                ...
                and not torch.cuda.get_device_name(0).endswith("[ZLUDA]")
                else torch.float32
            )
        except Exception:
            dtype = torch.float32
    # Normalize device for transformers.pipeline
...
def load_model(
    ...
    if vocab_file == "":
        # Try package resource path first then fall back to local repo path
        try:
            vocab_file = str(files("f5_tts").joinpath("infer/examples/vocab.txt"))
        except Exception:
            import os
            from pathlib import Path
            vocab_file = str(Path(os.path.dirname(__file__)).joinpath("examples", "vocab.txt"))
    tokenizer = "custom"
...
def infer_process(
    ...
    wav_np, sr = sf.read(ref_audio, always_2d=False)
    if wav_np.ndim == 1:
        wav_np = wav_np[None, :]
    else:
